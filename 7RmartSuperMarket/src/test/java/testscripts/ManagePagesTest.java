...
public class ManagePagesTest extends Base {
	@Test(groups= {"Regression"},retryAnalyzer=retry.Retry.class,description="This test case is to verify if an item can be deleted")
    ...
    managepage.navigatingToTheListPage(managepageurl).clickOnSearchIcon().enterTextInSearchField().clickOnSearchButton();

    	if (managepage.findingEnteredTextInTable()) 
    	{
    			managepage.clickingDeleteIcon();
    			boolean alertDeleteMessage = managepage.getDeleteMessage().contains("Page Deleted Successfully");
