...
public class AdminTest extends Base {
	@Test(groups = {
			"Regression" }, retryAnalyzer = retry.Retry.class, description = "This test case is to verify if a new admin user can be added")
	public void verifyIfAnAdminUserCanBeAdded() {
		...
		String sucesstext = ExcelUtility.getString(1, 4, "AdminPage");
		...
		adminpage.clickingNewButtonToLoadAdminUserInformationPage().enteringTextInUsernameField(newusername)
				.enteringTextInPasswordField(newpassword).choosingUserTypeFieldByVisibleTextMethod(dropdownvisibletext)
				.clickingOnSaveButton();
		boolean successAlertDisplay = adminpage.getSuccessMessage().contains(sucesstext);
		Assert.assertTrue(successAlertDisplay, "User was not created succesfully");
	}

	@Test(groups = {
			"Regression" }, retryAnalyzer = retry.Retry.class, description = "This test case is to verify if an existing user can be searched")
